...
public class StreamTest2 {
	public static void main(String[] args) {
		...
		// forEach() : 향상된 for문과 같이 앞의 iterator(순서가있는)요소를 각각 가져온 후 로직 작성
		// sorted() : 오름차순 정렬
...
//		// :: -> void거나 매개변수가 없는 경우 참조형 사용이 가능하다.
//		// 오름 차순
...
//		datas.stream().map(c-> c.toUpperCase().charAt(0)).forEach(c -> System.out.println(c));
		
		//-강사님의 답
//		datas.stream().map(c->c.charAt(0) >=97 && c.charAt(0) <= 122? c.replace(c.charAt(0), (char)(c.charAt(0)-32)) :c)
